...
def get_units(hbt_soft, raw_wav_path):
...
if __name__ == '__main__':
    dev = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # hubert的模型路径
    hbt_model = hubert_soft("../../checkpoints/hubert/hubert-soft.pt")
    # 这个不用改，自动在根目录下所有wav的同文件夹生成其对应的npy
    file_lists = get_end_file("../../batch", "wav")
    nums = len(file_lists)
    count = 0
